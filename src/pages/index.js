...
import { pushDL } from "../utils/track"
...
const IndexPage = () => {
  ...
  const firedDepth = useRef({ 25: false, 50: false, 75: false })
  useEffect(() => {
    const onScroll = () => {
      const h = document.documentElement
      const scrolled = (h.scrollTop / (h.scrollHeight - h.clientHeight)) * 100
      ;[25, 50, 75].forEach(pct => {
        if (!firedDepth.current[pct] && scrolled >= pct) {
          firedDepth.current[pct] = true
          pushDL({ event: "scroll_depth", percent: pct })
        }
      })
    }
  ...
  // ---- ENGAGED 30s
  useEffect(() => {
    const t = setTimeout(() => pushDL({ event: "engaged_30s" }), 30000)
    return () => clearTimeout(t)
  }, [])

  // ---- HANDLERS CTA
  const handleTelClick = (label = "tel_header", number = "+6282139702121") => {
    pushDL({ event: "tel_click", tel_label: label, tel_number: number })
    // lanjutkan default <a href="tel:...">
  }

  const handleWaClick = (
    label = "wa_header",
    number = "6282139702121",
    preset = "Hallo NBest, I Want To Book a Massage."
  ) => {
    pushDL({ event: "whatsapp_click", wa_label: label, wa_number: number })
    // redirect manual agar klik tetap berjalan setelah push
    if (typeof window !== "undefined") {
      window.location.href = `https://api.whatsapp.com/send?phone=${number}&text=${encodeURIComponent(
        preset
      )}`
    }
  }
      ...
      >
        <div className="sm:text-center md:text-left w-full">
          ...
          <div className="mt-3 text-base text-gray-800 md:text-gray-500 sm:mt-5 sm:text-lg sm:max-w-xl sm:mx-auto md:mt-5 md:text-xl lg:mx-0">
            <p>
              NBest Bali Massage provides home service to your palace with
              massage area in Bali
          ...
          <div className="mt-5 sm:mt-8 lg:flex lg:justify-start">
            <div className="rounded-md shadow">
              <a
                ...
                onClick={() => handleTelClick("tel_header", "+6282139702121")}
            ...
            <div className="mt-3 lg:mt-0 lg:ml-3">
              <a
                href="#wa" // dicegah; kita redirect manual di handler
                onClick={e => {
                  e.preventDefault()
                  handleWaClick("wa_header", "6282139702121")
                }}
      ...
      >
        <div className="mx-auto py-8 text-center">
          ...
          <div className="text-gray-700 mt-3 sm:mt-5 sm:text-lg md:mt-5 md:text-xl max-w-3xl lg:mx-auto">
            <p>
              <span className="font-extrabold text-black leading-relaxed">
                NBest bali massage
              </span>{" "}
              will give you a completely relaxing full body massage will make
              you feel fantastic . No matter what time of day or night you
              receive a message.
            </p>
            <div className="mt-3 leading-relaxed">
              ...
              room so that it is convenient and private. You no need to have to
              go to massage parlor or a spa to get the relaxation.
        ...
        <div className="mx-auto text-center py-8">
          ...
          <div className="text-gray-800 mt-3 sm:mt-5 sm:text-lg md:mt-5 md:text-xl max-w-3xl lg:mx-auto prose">
            <p>
              ...
              complete customer satisfaction to ensure repeat business. Our
              staff members are unique and highly skilled. They are equipped
              with the needed resources to help them satisfy you and give you an
            ...
            <div className="flex flex-col sm:flex-row justify-center mb-6 sm:mb-0">
              ...
              <div className="pb-8 shadow-lg flex-1 lg:flex-initial lg:w-1/4 rounded-lg rounded-l-none bg-gradient-to-r from-gray-500 to-white mt-4 flex flex-col">
                ...
                <div className="mt-8 text-lg text-gray-900 leading-relaxed space-y-4 px-2">
                  ...
                  <div className="px-2 mt-10 text-lg font-bold text-center prose">
                    We Also Provide For GROUP (Party, Sport Event, Vacation
                    Event ETC)
                  </div>
                </div>
              </div>
